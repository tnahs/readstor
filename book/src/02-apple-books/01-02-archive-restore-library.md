...
> [macOS - Library Location][macos-library-location] for more information.

However, archiving and restoring only these directories might miss some
...
> <i class="fa fa-exclamation-circle"></i> When restoring a library make
> sure that the current version of Apple Books for macOS and the version the
> archive was created from are identical. Not doing so could lead to unexpected
> results.
...
[macos-library-location]: ./01-01-library-location.md
