...
/// the [`Registry`] it needs to be validated both for its syntax and for the
/// fields that its variables reference. In order to achieve the latter, a dummy
/// [`Entry`] struct---its [`Default`] implementation---is passed to validate
/// the template's variables. Seeing as `DateTime` does not have a [`Default`]
/// implementation, it was either we implementation a hand written [`Default`]
/// of [`Entry`] which would include multiple nested structs or wrap
/// [`DateTime<Utc>`] and provide a [`Default`] implementation.
///
/// See [`Registry::add()`] for more information.
